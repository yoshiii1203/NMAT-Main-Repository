...
**Cause:** 3,069 records with `NMS_PER_num` but missing `PercentileBin` pull the median down by 1 point. Dashboard is correct — it includes all records with score data. The compute script should not filter on `PercentileBin` for median computation.

### Pre-computed Globals Verified

| Global | Dashboard | Verifier | Status |
|--------|:---------:|:--------:|:------:|
| PUB_B5_RATE | 64.9% | 64.9% | ✅ |
| PUB_B5_COUNT | 17,482 | 17,482 | ✅ |
| PUB_TOTAL | 26,937 | 26,937 | ✅ |
| PUB_B4O_RATE | 8.3% | 8.3% | ✅ |
| N_CLEAN_B5 | 23,357 | 23,357 | ✅ |
| N_CLEAN_PLE | 27,151 | 27,151 | ✅ |
| N_FOREIGN_ALL | 32,501 | 32,501 | ✅ |
| N_FILIPINO_ALL | 146,413 | 146,413 | ✅ |
...
### ⚠️ Column dtype bug: HasTRUErawScores / StoredVsDerivedMismatch

The parquet stores `HasTRUErawScores` as **string** (`"True"` / `"False"`) and `StoredVsDerivedMismatch` as **string** (`"1.0"` / `"0.0"` / `"NaN"`). The dashboard compares them with Python `== True` and `== 1.0`, which always evaluates to `False` since `"True" != True` and `"1.0" != 1.0`.

**Actual correct values (if normalized):**
- `HasTRUErawScores` = True: **178,882** (dashboard shows 0)
- `StoredVsDerivedMismatch` = 1.0: **56,065** (dashboard shows 0)
- `CalcVsDerivedMismatch` = 1.0: **0** (dashboard shows 0 — this is correct)

This is a **low-impact** bug — the counts only appear in the Tab 6 expander and are not used in any analytical computation.
...
| Priority | Issue | Tab | Suggested Fix |
|----------|-------|:---:|--------------|
| 🟢 Low | `HasTRUErawScores` dtype mismatch | 6 | Add string-to-bool coercion in `validate_schema()` for `HasTRUErawScores` |
| 🟢 Low | `StoredVsDerivedMismatch` dtype | 6 | Add string-to-float coercion in `validate_schema()` |
| 🟢 Low | India count off by 1 | 4 | Compute script should use `Verified Foreigner` only (match dashboard) |
| 🟢 Low | Median bin rank off by 1 | 5 | Compute script should not filter on `PercentileBin` for median computation |
| 🟢 Low | Find 2 wording in compute script | 5 | Remove PercentileBin dropna before median computation |
| 🟢 Low | Page_results filenames stale | All | Already fixed with SCRIPT constant update |

**No critical errors found.** The dashboard's displayed values are correct. All discrepancies are in the compute scripts, not the dashboard.

---

*Generated by 6 parallel verifier agents. Each value cross-checked against both `page_results/*.md` and direct parquet computation.*

# Cut-Off Scenario Analysis: 30th vs 40th Percentile

**Date:** July 28, 2026
**Data Source:** `NMAT_Exodus.parquet` (178,927 records, 54 columns)
**Script:** `ched_compute/02_cutoff_scenarios.py`

---

## Results

This section models the impact of two proposed NMAT cut-off thresholds: the 30th percentile (B4+) and the 40th percentile (B5+). For each threshold, we compute the number and proportion of examinees who qualify, broken down by university type, year, and PLE linkage rate.
...
| **At or Above B4 (30th+ Percentile)** | 91,409 |
| **At or Above B5 (40th+ Percentile)** | 78,944 |
...
### Cut-Off Impact by University Type

Table shows the number and percent of examinees qualifying at each cut-off threshold, by university type.

| UNI_TYPE | n (Best) | B4+ (30th+) | % B4+ | B5+ (40th+) | % B5+ | B4 Range (30-39th) |
|:---------|:--------:|:-----------:|:-----:|:-----------:|:-----:|:------------------:|
...
### Cut-Off Impact by Year

Table shows year-by-year qualifying counts at each threshold.

| Year | n (Best) | B4+ (30th+) | % B4+ | B5+ (40th+) | % B5+ | B4 Range |
|:----:|:--------:|:-----------:|:-----:|:-----------:|:-----:|:--------:|
...
### PLE Linkage Rate by Cut-Off and University Type

For each university type and cut-off threshold, the NMAT-to-PLE linkage rate among the pre-2015 cohort.

| UNI_TYPE | B4+ (30th+) Linkage | B5+ (40th+) Linkage | Gap (pp) |
|:---------|:-------------------:|:-------------------:|:--------:|
...
### Linkage Rate by Individual Percentile Bin

Shows how the linkage rate changes across each percentile bin, highlighting the critical B4→B5 transition.

| PercentileBin | Percentile Range | n (Pre-2015) | PLE Matched | Linkage Rate |
|:------------:|:----------------:|:------------:|:-----------:|:------------:|
...
The sharpest bin-to-bin increase occurs between B4 (30th–39th) and B5 (40th–49th). This is the empirical basis for the tiered cut-off proposal: the 40th percentile threshold selects a pool with meaningfully higher PLE linkage outcomes.

### Interpretation

Among 133,804 NMAT examinees (best record):
- **91,409 (68.3%)** qualify at the 30th percentile cut-off (B4+)
- **78,944 (59.0%)** qualify at the 40th percentile cut-off (B5+)
- **12,465 examinees (9.3%)** fall in the B4 range (30th–39th) — 
  the marginal pool affected by choosing the higher cut-off

The PLE linkage rate at B4+ is 56.45% vs 61.17% at B5+ — a difference of 4.72 percentage points. This supports the tiered approach: the 40th threshold meaningfully differentiates applicant pools on PLE linkage outcomes.
